...
进入工作流后，以 `.claude/workflows/collab_v2.json` 为机器契约源。必须真实调用对应 Subagent、执行阶段产物的存在性与语义门禁，并遵守用户确认节点；不得用口头声称代替文件和验证结果。事实核查结果必须绑定当前正文文件与 SHA-256，自动清稿必须明确传入项目，Humanizer 不得新增用户素材和证据账本之外的经历或事实。
