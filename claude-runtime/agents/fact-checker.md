...
```bash
python "{{WRITING_AGENT_SCRIPTS}}/update_run_manifest.py" --project "[项目名]" --body "[最终正文文件]" --status fact-checked --workflow-version collab-v2 --fact-check-status passed --fact-claims fact_claims.json --fact-report fact_check_report.md
```

如果有红色问题，执行：

```bash
python "{{WRITING_AGENT_SCRIPTS}}/update_run_manifest.py" --project "[项目名]" --body "[最终正文文件]" --status fact-check-blocked --workflow-version collab-v2 --fact-check-status blocked --fact-claims fact_claims.json --fact-report fact_check_report.md
```
