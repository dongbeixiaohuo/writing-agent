...
```bash
python "{{WRITING_AGENT_SCRIPTS}}/verify_required_files.py" --project "[项目名]" --workflow ".claude/workflows/collab_v2.json" --stage 11 --mode B
...
```bash
python "{{WRITING_AGENT_SCRIPTS}}/update_run_manifest.py" --project "[项目名]" --body draft_vN_illustrated.md --notes draft_vN_illustrated_notes.md --status illustrated --workflow-version collab-v2
```
