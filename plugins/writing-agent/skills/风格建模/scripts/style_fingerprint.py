...
用法：
  ...
  python .claude/skills/风格建模/scripts/style_fingerprint.py --json docs/样本1.md
"""
...
def extract_body_lines(text: str):
    """去掉元数据行、标题行、表格、分割线，保留正文行。"""
    lines = []
    for line in text.splitlines():
        stripped = line.strip()
        if not stripped or META_LINE.match(stripped):
            continue
        # 去除加粗/斜体标记
...
def analyze(paths):
    ...
    ellipsis_per_k = (full_text.count('…') + full_text.count('......') + full_text.count('。。。')) / total_chars * 1000
    dash_per_k = (full_text.count('——') + full_text.count('—')) / total_chars * 1000
